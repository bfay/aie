...
class WPDD_Layouts_PostTypesManager
{
	...
	public function __construct()
	{
		...
		// debug only
	//	add_filter ( 'page_template', array (&$this, 'post_type_single_template' ), 99, 1 );
	//	add_filter ( 'single_template', array (&$this, 'post_type_page_template' ), 99, 1 );

		if (isset($_GET['page']) and $_GET['page']=='dd_layouts_edit') {
			add_action('admin_enqueue_scripts', array($this, 'load_editor_script') );
		}
	}
	// debug only
	...
	public function load_editor_script()
	{
		...
		$wpddlayout->localize_script('ddl-post-type-options-script', 'DDLayout_options', array(
				self::DDL_POST_TYPES_OPTIONS => array(
	...
	private function get_post_types_from_wp()
	{
		$args = array(
			'public'   => true,
			//'_builtin' => false
		...
		if ($post_types_obj[$layout[0]]) {
			...
			foreach( $post_types as $post_type )
			{
				...
				if ($check !== null) {
					...
					//$this->track_batched_post_types( $post_type, $layout_id );
				}
			}
		}
		else
	...
	private function update_post_meta_for_post_type( $posts, $layout_id )
	{
		$ret = array();
	...
	public function set_layout_for_post_type_meta_callback()
	{
		if( $_POST && wp_verify_nonce( $_POST['set-layout-for-cpt-nonce'], 'set-layout-for-cpt-nonce' ) )
		{
			...
			{
				$send = json_encode( array('message' => $data) );
			}
		}
		else
		{
	...
	public function print_post_types_checkboxes( $current = false, $do_not_show = false, $id_string = "" )
	{
	...
	public function set_change_layout_usage_box( )
	{
		if( $_POST && wp_verify_nonce( $_POST['layout-select-set-change-nonce'], 'layout-select-set-change-nonce' ) )
		{
			...
			$send = json_encode( array( 'message' => array( 'html_data' => $this->print_post_types_checkboxes( $_POST['layout_id'], true, 'change' ), 'nonce' => $nonce, 'layout_id' => $_POST['layout_id'] ) ) );
		}
		else
		{
			$send = json_encode( array( 'error' =>  __( sprintf('Nonce problem: apparently we do not know from where the request comes from. %s', __METHOD__ ), 'ddl-layouts') ) );
	...
	public function set_layouts_post_types_on_usage_change_js()
	{
		...
		if( $_POST && wp_verify_nonce( $_POST['layout-set-change-post-types-nonce'], 'layout-set-change-post-types-nonce' ) )
		{
			...
			$this->handle_post_type_data_save( array( "layout_".$_POST['layout_id'] => $post_types ) );
			//$send = json_encode( array( 'message'=> array( 'changed' =>$this->get_layout_post_types_object($_POST['layout_id']), 'done' => 'yes' ) ) );
			$status = isset($_GET['status']) && $_GET['status'] === 'trash' ? $_GET['status'] : 'publish';
			
			$data = $wpddlayout->listing_page->get_grouped_layouts( $status );
