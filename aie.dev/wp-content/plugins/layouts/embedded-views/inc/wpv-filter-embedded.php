            ...
            case 'checkboxes':
                ...
                for( $i = 0; $i < $count_values_array; $i++ ) {
                    ...
                    if ( $dependant ) {
						...
						if ( empty( $value ) && is_object( $this_query ) ) {
						//	$this_query = $WP_Views->get_query();
						...
						if ( !$this_checker && !empty( $value ) && !$options[$value]['#default_value'] ) {
            ...
            default:
                ...
                for( $i = 0; $i < $count_values_array; $i++ ) {
					...
					if ( $dependant ) {
						...
						if ( !$this_checker && !empty( $value ) ) { // TODO DONE need to merge this with the default_value below, to avoid hiddin or disabling selected items
			...
			foreach ( $field_radio_options as $key => $opts ) {
				if ( is_array( $opts ) ) {
					...
					if ( $dependant ) {
						...
						if ( !$this_checker && !empty( $value ) && ( !isset( $_GET[$url_param] ) || $_GET[$url_param] !== $value ) ) {
			...
			foreach ( $field_select_options as $key => $opts ) {
				if ( is_array( $opts ) ) {
					...
					if ( $dependant ) {
						...
						if ( !$this_checker && !empty( $value ) ) { // TODO DONE we need to adjust this with the $default_value below
							$options[$display_value]['#disable'] = 'true';
							$options[$display_value]['#labelclass'] = 'wpv-parametric-disabled';
							if ( $multi == 'multiple' ) {
								if ( isset( $empty_action['multi-select'] ) && $empty_action['multi-select'] == 'hide' ) {
