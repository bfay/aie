...
function wpcf_cd_post_groups_filter( $groups, $post, $context ) {
    ...
    foreach ( $groups as $key => &$group ) {
        ...
        if ( defined( 'WPTOOLSET_FORMS_VERSION' ) ) {
            $conditions = !isset( $group['conditional_display'] ) ? get_post_meta( $group['id'],
            ...
            if ( !empty( $conditions['conditions'] ) ) {
                ...
                if ( !$passed ) {
                    wpcf_cd_add_group_js( 'add', $meta_box_id );
...
function wpcf_cd_add_group_js( $call, $field = false, $value = false,
    ...
    wpcf_cd_add_group_js_render( array_keys( $conditions ) );
}
...
function wpcf_cd_add_group_js_render( $conditions = array() ) {
    if ( defined( 'WPTOOLSET_FORMS_VERSION' ) ) {
        if ( !empty($conditions) ):
        ?>
    <style type="text/css">#<?php echo implode(',#', $conditions); ?>{display:none;}</style>
    <?php
        endif;
        return;
    }
